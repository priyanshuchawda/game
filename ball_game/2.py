...
def get_brick_color(hits):
    """Return a color based on the brick's hit points."""
    if hits >= 3:
        return YELLOW
    elif hits == 2:
        return ORANGE
...
def create_bricks():
    ...
    The top rows are tougher, so durability decreases with each row.
    Each brick is stored as a dictionary with keys: rect, hits, and color.
    """
    global bricks
    bricks = []
    for row in range(brick_rows):
        # For a simple variation, set hits based on the row number.
        # The top row (row 0) gets more hits.
        hits = 3 - row if 3 - row > 0 else 1
        for col in range(brick_cols):
            ...
            brick = {"rect": rect, "hits": hits, "color": get_brick_color(hits)}
            bricks.append(brick)
...
while running:
    ...
    if not paused and not game_over:
        ...
        # Instead of removing immediately, check and reduce brick durability.
        for brick in bricks:
            if brick["rect"].colliderect(ball_rect):
                ...
                if brick["hits"] <= 0:
                    ...
                    brick["color"] = get_brick_color(brick["hits"])
